...
public void analyze(loc datadir, loc resultdir){
	...
	dependencies = { dependencies(file.extension, readFile(file)) | file <- files };
	for(file <- files){
		...
		/*switch(file.extension){
	    		...
	    		case "gradles":{
	    			...
	    			cmtest = addAll(cmtest, testArtifactsFromGradle(content));
	    		}
	    		case "ivys": {
	...
	cmtest = (k:cmtest[k] | k <- cmtest, !contains(k, "maven"), !contains(k, "gradle"), !contains(k, "ivy")); // remove all build-style dependencies
	
	sorted = sort(toList(cm), sortOnSnd);
	sorted2 = sort(toList(cmtest), sortOnSnd);
	//iprintln(sorted);
	...
	makeFrequencyFile(resultdir + "/frequency2.tsv", sorted2);
}

private set[Dependency] dependencies(/'gradles'/, str content) = 
